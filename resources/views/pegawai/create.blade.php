...
@section('header_title', 'Tambah Pegawai Baru')

@section('content')
<div class="card" style="max-width: 1000px; margin: 0 auto;">
    <div style="border-bottom: 1px solid var(--border-color); padding-bottom: 20px; margin-bottom: 32px;">
        <h3 style="font-weight: 600; font-size: 1.25rem;">Formulir Profil Pegawai Baru</h3>
        <p style="font-size: 0.85rem; color: var(--text-secondary); margin-top: 4px;">Isi seluruh data profil dengan lengkap. Menambahkan pegawai secara otomatis akan membuatkan akun User dengan username = NIK dan password default = password123.</p>
    </div>
        ...
        <div style="margin-bottom: 32px;">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="nik" name="nik" class="form-control" value="{{ old('nik') }}" placeholder="Contoh: 1003" required>
                </div>
                <div class="form-group">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="no_ktp" name="no_ktp" class="form-control" value="{{ old('no_ktp') }}" placeholder="16 digit nomor KTP" required>
                </div>
                <div class="form-group">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="tmp_lahir" name="tmp_lahir" class="form-control" value="{{ old('tmp_lahir') }}" placeholder="Tempat lahir" required>
                </div>
                <div class="form-group">
            ...
            <div class="form-row">
                <div class="form-group" style="grid-column: span 2;">
                    ...
                    <input type="text" id="alamat" name="alamat" class="form-control" value="{{ old('alamat') }}" placeholder="Masukkan alamat lengkap rumah" required>
                </div>
                <div class="form-group">
                    <label class="form-label" for="kota">Kota *</label>
                    <input type="text" id="kota" name="kota" class="form-control" value="{{ old('kota') }}" placeholder="Kota tempat tinggal" required>
                </div>
            </div>
        </div>
        ...
        <div style="margin-bottom: 32px;">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="jbtn" name="jbtn" class="form-control" value="{{ old('jbtn') }}" placeholder="Contoh: Perawat Gigi, Staf Keuangan" required>
                </div>
                <div class="form-group">
                    ...
                    <select id="jnj_jabatan" name="jnj_jabatan" class="form-control" required>
                        ...
                        @foreach($positions as $pos)
                            <option value="{{ $pos->kode }}" {{ old('jnj_jabatan') === $pos->kode ? 'selected' : '' }}>{{ $pos->nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select id="kode_kelompok" name="kode_kelompok" class="form-control" required>
                        ...
                        @foreach($groups as $grp)
                            <option value="{{ $grp->kode_kelompok }}" {{ old('kode_kelompok') === $grp->kode_kelompok ? 'selected' : '' }}>{{ $grp->nama_kelompok }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="form-group">
                    ...
                    <select id="departemen" name="departemen" class="form-control" required>
                        ...
                        @foreach($departments as $dept)
                            <option value="{{ $dept->dep_id }}" {{ old('departemen') === $dept->dep_id ? 'selected' : '' }}>{{ $dept->nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select id="bidang" name="bidang" class="form-control" required>
                        ...
                        @foreach($bidangList as $bd)
                            <option value="{{ $bd->nama }}" {{ old('bidang') === $bd->nama ? 'selected' : '' }}>{{ $bd->nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select id="indexins" name="indexins" class="form-control" required>
                        ...
                        @foreach($departments as $dept)
                            <option value="{{ $dept->dep_id }}" {{ old('indexins') === $dept->dep_id ? 'selected' : '' }}>{{ $dept->nama }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="form-group">
                    ...
                    <select id="stts_kerja" name="stts_kerja" class="form-control" required>
                        ...
                        @foreach($workStatuses as $ws)
                            <option value="{{ $ws->stts }}" {{ old('stts_kerja') === $ws->stts ? 'selected' : '' }}>{{ $ws->ktg }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select id="stts_aktif" name="stts_aktif" class="form-control" required>
                        ...
                        <option value="CUTI" {{ old('stts_aktif') === 'CUTI' ? 'selected' : '' }}>CUTI</option>
                        <option value="KELUAR" {{ old('stts_aktif') === 'KELUAR' ? 'selected' : '' }}>KELUAR</option>
                        <option value="TENAGA LUAR" {{ old('stts_aktif') === 'TENAGA LUAR' ? 'selected' : '' }}>TENAGA LUAR</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <select id="ms_kerja" name="ms_kerja" class="form-control" required>
                        <option value="<1" {{ old('ms_kerja') === '<1' ? 'selected' : '' }}>Kurang dari 1 tahun (&lt;1)</option>
                        <option value="PT" {{ old('ms_kerja') === 'PT' ? 'selected' : '' }}>Masa Percobaan (PT)</option>
                        <option value="FT>1" {{ old('ms_kerja') === 'FT>1' ? 'selected' : '' }}>Pegawai Tetap &gt; 1 Tahun (FT&gt;1)</option>
                    </select>
                </div>
                <div class="form-group">
                    <label class="form-label" for="wajibmasuk">Hari Wajib Masuk (Bulanan) *</label>
                    <input type="number" id="wajibmasuk" name="wajibmasuk" class="form-control" value="{{ old('wajibmasuk', 25) }}" min="1" max="31" required>
                </div>
            </div>
        </div>
                ...
                <div class="form-group">
                    ...
                    <select id="stts_wp" name="stts_wp" class="form-control" required>
                        ...
                        @foreach($taxStatuses as $ts)
                            <option value="{{ $ts->stts }}" {{ old('stts_wp') === $ts->stts ? 'selected' : '' }}>{{ $ts->ktg }} ({{ $ts->stts }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label class="form-label" for="npwp">Nomor NPWP</label>
                    <input type="text" id="npwp" name="npwp" class="form-control" value="{{ old('npwp') }}" placeholder="Masukkan NPWP (opsional)">
                </div>
                <div class="form-group">
                    ...
                    <select id="pendidikan" name="pendidikan" class="form-control" required>
                        ...
                        @foreach($educations as $edu)
                            <option value="{{ $edu->tingkat }}" {{ old('pendidikan') === $edu->tingkat ? 'selected' : '' }}>{{ $edu->tingkat }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="bpd" name="bpd" class="form-control" required>
                        ...
                        @foreach($banks as $b)
                            <option value="{{ $b->namabank }}" {{ old('bpd') === $b->namabank ? 'selected' : '' }}>{{ $b->namabank }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label class="form-label" for="rekening">Nomor Rekening *</label>
                    <input type="text" id="rekening" name="rekening" class="form-control" value="{{ old('rekening') }}" placeholder="Nomor rekening bank" required>
                </div>
                <div class="form-group">
                    <label class="form-label" for="gapok">Gaji Pokok (IDR) *</label>
                    <input type="number" id="gapok" name="gapok" class="form-control" value="{{ old('gapok') }}" placeholder="Gaji pokok nominal" required>
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="kode_resiko" name="kode_resiko" class="form-control" required>
                        ...
                        @foreach($risks as $r)
                            <option value="{{ $r->kode_resiko }}" {{ old('kode_resiko') === $r->kode_resiko ? 'selected' : '' }}>{{ $r->nama_resiko }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select id="kode_emergency" name="kode_emergency" class="form-control" required>
                        ...
                        @foreach($emergencyIndexes as $em)
                            <option value="{{ $em->kode_emergency }}" {{ old('kode_emergency') === $em->kode_emergency ? 'selected' : '' }}>{{ $em->nama_emergency }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
